...
| **RISK-009** | Migrations table is out of sync with the real schema | med | **occurred** | james | **Measured 2026-07-24: 31 tables in production, 17 from migrations, 6 from Passport — leaving 8 application tables created entirely outside Laravel** (`testmaker_list`, `testmaker_list_restaurant`, `bookmark_testmaker_list`, `restaurant_images`, `imagelike`, `badge_categories`, `api_logs`, `seed_logs`). `artisan migrate` cannot be trusted to reproduce this schema. → TASK-14 |
