...
| **TASK-20** | Photo upload is off. `restaurant-image-save` writes to `public_path('storage/res_image')` — Railway's filesystem is ephemeral, so uploads vanish on restart. Needs object storage. **Also why every legacy profile image 404s.** **CORRECTED 2026-08-19:** the ephemeral disk is real but SECONDARY — the endpoint 500s on its first statement, because `logAdd` wrote four column names `api_logs` does not have. Fixed in backend PR #38, so uploads now reach the filesystem and *then* hit this. Validation was also absent (`"image" => ""`); now mimes + size limited. What remains here is genuinely the storage. | Photos | p1 | `open` |
...
| **TASK-02** | Add an ownership check to `tastemaker_listdelete` — currently any authenticated user can delete any list by guessing an integer id **DONE 2026-08-19 (backend PR #19).** `tastemaker_listdelete` now requires the list to belong to `Auth::id()`; a list owned by someone else and one that does not exist both return 404, so it cannot be used to probe ids. | RM-02 | p1 | `done` |
| **TASK-03** | Replace body `user_id` with `Auth::id()` in `tagsdelete` — the add path authenticates, the delete path doesn't **DONE 2026-08-19 (backend PR #19).** `tagsdelete` uses `Auth::id()`. The body `user_id` is still accepted so the shipped iOS build keeps validating, but it is ignored. | `PRD-001` §9, RM-02 | p1 | `done` |
...
| **TASK-10** | Replace 19 MySQL-only `IFNULL`/`IF()` calls with `COALESCE`/`CASE` — they throw on PostgreSQL, so `getallBadges` and others break on Supabase **PARTIAL 2026-08-19:** the `IFNULL`s inside `getallBadges` are fixed (`COALESCE`), which unblocked that endpoint. **17 remain in UserController alone** — each belongs to a different endpoint and needs its own verification. **DONE 2026-08-19 (backend PR #22).** All 36 `IFNULL` → `COALESCE`, and the 2 MySQL `IF()` sites → `CASE WHEN` (the tag-level expression; thresholds preserved: >4→1 … >1→4, else 5). Zero MySQL-only SQL constructs remain in `app/`. NOTE: making that query *run* did not make it *correct* — see todo 121. | RM-13, RM-01 | p1 | `done` |
...
| **TASK-13** | Determine what `/restaurants1` is — live, experimental, or dead. Dead code carried through module extraction costs real effort. **DONE 2026-08-19 (backend PR #23).** `/restaurants1` was dead AND actively broken: the route was registered while `getRestaurants1()` sat commented out, so it returned 500 on every call. Route plus 252 commented lines removed; it 404s now. | RM-05 | p3 | `done` |
