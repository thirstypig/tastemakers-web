...
const TODO_ITEMS: TodoItem[] = [
  ...
  },
  {
    id: "web-real-data",
    platform: "web",
    title: "Decide whether web keeps reading Supabase directly",
    phase: "P2",
    status: "open",
    detail: "Superseded 2026-08-20. This used to read 'swap the stub layer for real Laravel API data' — there is no stub layer; src/lib/api/index.ts and every features/*/api.ts query Supabase directly and have for months. The real open question is TODO-089: web and the mobile clients read the same database through entirely different code, so the tag-ranking rule now exists in four places and a schema change breaks web without touching the API contract mobile depends on. Upside is that Laravel being down does not take the website down. This is an architecture decision, not a migration task.",
  ...
  },
  {
    id: "web-known-for",
    platform: "web",
    title: "Populate or remove the 'Known for' tag cloud on tastemaker profiles",
    phase: "P3",
    status: "open",
    detail: "BLOCKED ON A PRODUCT DECISION, not on code. The section renders behind tastemaker.tags.length > 0 and both listTastemakers() and getTastemaker() return a hardcoded empty array, so it has never displayed. The page can say someone has 932 tags while being structurally unable to show one. Populating it needs the ranking answered first: community consensus (how many people applied a tag, per project_tag_popularity) or personal frequency (how often THIS person used it). Those are different queries and the second is probably right on a person's profile. Do not 'fix' by hiding the stat — the count is real, the tags are what is missing. Todo 125.",
    file: "src/lib/api/index.ts",
    date: "",
  },
  {
    id: "web-profile-url-scheme",
    platform: "web",
    title: "Retire the numeric-id fallback in tastemaker profile URLs",
    phase: "P3",
    status: "open",
    detail: "The casing half shipped 2026-08-20 (PR #56): lowercase slugs now resolve and 308 to the canonical spelling. Still open — slug is built as username ?? String(id), so a tastemaker with no username gets /tastemakers/73, and getTastemaker branches on isNaN(parseInt(slug)), meaning an all-digits username would route to the id lookup and miss. No such username exists today; nothing prevents one. Restaurants and lists already use name-plus-id slugs, so the site has two URL schemes for two of its three content types. Todo 128.",
    file: "src/lib/api/index.ts",
    date: "",
  },
  // ── Marketing P1 ────────────────────────────────
